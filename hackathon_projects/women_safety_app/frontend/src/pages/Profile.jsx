...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    if (!u.pin) {
      setUnlocked(true); // if no PIN set, allow access
    }
    setName(u.name || "");
    setPhone(u.phone || "");
  ...
  const save = () => {
    ...
    const updated = {
      ...
      silent,
      pin: newPin.length === 4 ? newPin : user.pin, // ✅ change PIN if entered
    };
    ...
    <div className="min-h-[calc(100vh-56px)] flex items-start justify-center bg-gray-900 text-white">
      <div className="bg-gray-800 w-full max-w-xl mt-10 p-8 rounded-2xl shadow-lg">
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          {/* Change PIN */}
          <div>
            <label className="block text-sm font-semibold mb-1">Change PIN (optional)</label>
            <input
              type="password"
              maxLength={4}
              pattern="\d*"
